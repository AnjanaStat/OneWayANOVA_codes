...
# library Iso is needed for finding the pava function
library(Iso)
# by fun1 and fun2 we find critical value
fun1<-function(n1,n2,n3,n4,v1,v2,v3,v4)
{
  # generating sample from normal distribution with mean 0 and variance v_i's, calculated from fun3
  X1=rnorm(n1,0,sqrt(v1))
  X2=rnorm(n2,0,sqrt(v2))
  X3=rnorm(n3,0,sqrt(v3))
  X4=rnorm(n4,0,sqrt(v4))
  # below steps are for finding coefficient of the polynomial of common mean
  m1<-mean(X1)
  m2<-mean(X2)
  m3<-mean(X3)
  m4<-mean(X4)
  ...
  n=length(Rert)
  # find the n values of sigma_1^2 
  m7=NULL
  for(i in 1:n)
  {
      m7[i]=(sum((X1-Rert[i])^2))/n1
  }
  # gives MLE of sigma_1^2
  m71=min(m7)
  # find the n values of sigma_2^2 
  m8=NULL
  for(i in 1:n)
  {
      m8[i]=(sum((X2-Rert[i])^2))/n2
  }
  # find MLE of sigma_2^2
  m81=min(m8)
  # find the n values of sigma_3^2 
  m9=NULL
  for(i in 1:n)
  {
      m9[i]=(sum((X3-Rert[i])^2))/n3
  }
  # Gives MLE of sigma_3^2
  m91=min(m9)
  # find the n values of sigma_4^2 
  m10=NULL
  for(i in 1:n)
  {
      m10[i]=(sum((X4-Rert[i])^2))/n4
  }
  # gives MLE of sigma_4^2
  m101=min(m10) 
  # below steps are for finding MLEs of the parameters under ordered alternative space
  # x8 gives MLEs of mu_i's and S0 gives MLEs of sigma_i^2
  x<-c(m1, m2, m3, m4)
  s1<-sum((X1-m1)^2)/n1
  s2<-sum((X2-m2)^2)/n2
  s3<-sum((X3-m3)^2)/n3
  ...
  dno<-(m71)^(n1/2)*(m81)^(n2/2)*(m91)^(n3/2)*(m101)^(n4/2)
  # find LR value
  value=neu/dno
  return(value)
}
...
{ 
  # generate sample from normal distribution with given means variances
  X1=rnorm(n1,mu1,sqrt(v11))
  X2=rnorm(n2,mu2,sqrt(v12))
  X3=rnorm(n3,mu3,sqrt(v13))
  X4=rnorm(n4,mu4,sqrt(v14))
  # steps 138-182 for finding coefficients of the polynomial
  m1<-mean(X1)
  m2<-mean(X2)
  m3<-mean(X3)
  m4<-mean(X4)
  ...
  d5<-h3-2*h4*m3+p3
  d6<-h4-2*m3
  # steps 185-192 give coefficients of the polynomial
  z1<-n1*m1*a1+n2*m2*b1+n3*m3*c1+n4*m4*d1
  z2<-n1*(m1*a2-a1)+n2*(m2*b2-b1)+n3*(m3*c2-c1)+n4*(m4*d2-d1)
  z3<-n1*(m1*a3-a2)+n2*(m2*b3-b2)+n3*(m3*c3-c2)+n4*(m4*d3-d2)
  z4<-n1*(m1*a4-a3)+n2*(m2*b4-b3)+n3*(m3*c4-c3)+n4*(m4*d4-d3)
  ...
  z7<-n1*(m1-a6)+n2*(m2-b6)+n3*(m3-c6)+n4*(m4-d6)
  z8<--(n1+n2+n3+n4)
  # give roots of the polynomial
  root<-polyroot(c(z1,z2,z3,z4,z5,z6,z7,z8))
  # find the real roots of the polynomial
  Rert=Re(root)[abs(Im(root))<=0.0001]
  # find number of real roots 
  n=length(Rert)
  # find the n values of sigma_1^2 corresponding to the real values of mu
  m7=NULL
  for(i in 1:n)
  {
  ...
  # gives MLe of sigma_1^2 
  m71=min(m7) 
  # find the n values of sigma_2^2 corresponding to the real values of mu
  m8=NULL
  for(i in 1:n)
  {
  ...
  # find the MLE of sigma_2^2
  m81=min(m8)
  # find the n values of sigma_3^2 corresponding to the real values of mu
  m9=NULL
  for(i in 1:n)
  {
  ...
  # gives MLE of sigma_3^2
  m91=min(m9)
  # find the n values of sigma_4^2 corresponding to the real values of mu
  m10=NULL
  for(i in 1:n)
  {
    m10[i]=(sum((X4-Rert[i])^2))/n4
  ...
  #find MLE of sigma_4^2
  m101=min(m10) 
  # steps 232-259 are for finding the MLEs of the parameters over the alternative space
  # x8 gives MLEs of mu_i's and s0 gives MLEs of sigma_i^2
  x<-c(m1, m2, m3, m4)
  s1<-sum((X1-m1)^2)/n1
  s2<-sum((X2-m2)^2)/n2
  ...
  # find the critical value from fun2
  out=fun2(n1,n2,n3,n4,v1,v2,v3,v4)
  neu<-(s0[1])^(n1/2)*(s0[2])^(n2/2)*(s0[3])^(n3/2)*(s0[4])^(n4/2)
  dno<-(m71)^(n1/2)*(m81)^(n2/2)*(m91)^(n3/2)*(m101)^(n4/2)
  # gives the LR value
  value=neu/dno
