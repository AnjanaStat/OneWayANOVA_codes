...
rm(list=ls())
# package Iso is needed for pava function
library(Iso)
# using fun1 and fun2 we find critical value
fun1<-function(n1,n2,n3,n4,n5,v1,v2,v3,v4,v5)
{
  ...
  g4<-rnorm(n4,0,sqrt(v4))
  g5<-rnorm(n5,0,sqrt(v5))
  # below steps are for finding coefficients of the polynomial of common mean mu
  m1<-mean(g1);m2<-mean(g2);m3<-mean(g3); m4<-mean(g4);m5<-mean(g5)
  p1<-sum(g1^2)/n1; p2<-sum(g2^2)/n2;p3<-sum(g3^2)/n3;p4<-sum(g4^2)/n4;p5<-sum(g5^2)/n5
  a1<-p2*p3
  a2<--2*(m3*p2+p3*m2)
  a3<-p3+4*m2*m3+p2
  a4<--2*(m2+m3)
  ...
  Rert=Re(root)[abs(Im(root))<=0.0001]
  # number of real roots
  n=length(Rert)
  # find the values of sigma_1^2 corresponding to the real roots
  m8=NULL
  for(i in 1:n)
  {
    m8[i]=(sum((g1-Rert[i])^2))/n1
  }
  # gives the MLE of sigma_1^2
  m81=min(m8)
  # find the values of sigma_2^2 corresponding to the real roots
  m9=NULL
  for(i in 1:n)
  {
    m9[i]=(sum((g2-Rert[i])^2))/n2
  }
  # gives the MLE of sigma_2^2
  m91=min(m9)
  # find the values of sigma_3^2 corresponding to the real roots
  m10=NULL
  for(i in 1:n)
  {
    m10[i]=(sum((g3-Rert[i])^2))/n3
  }
  # gives the MLE of sigma_3^2
  m101=min(m10)
  # find the values of sigma_4^2 corresponding to the real roots
  m11=NULL
  for(i in 1:n)
  {
    m11[i]=(sum((g4-Rert[i])^2))/n4
  }
  # gives the MLE of sigma_4^2
  m111=min(m11)
  # find the values of sigma_5^2 corresponding to the real roots
  m12=NULL
  for(i in 1:n)
  {
    m12[i]=(sum((g5-Rert[i])^2))/n5
  }
  # gives the MLe of sigma_5^2
  m121=min(m12)
  # below steps for finding MLEs of the parameters under ordered alternative space
  x0<-c(m1, m2, m3, m4, m5)
  s21<-sum((g1-m1)^2)/n1
  s22<-sum((g2-m2)^2)/n2
  s23<-sum((g3-m3)^2)/n3
  ...
  neu<-(s0[1]^(n1/2))*(s0[2]^(n2/2))*(s0[3]^(n3/2))*(s0[4]^(n4/2))*(s0[5]^(n5/2))
  dno<-(m81^(n1/2))*(m91^(n2/2))*(m101^(n3/2))*(m111^(n4/2))*(m121^(n5/2))
  # gives LR value
  value=neu/dno
  return(value)
}
...
fun3<-function(n1,n2,n3,n4,n5,mu1,mu2,mu3,mu4,mu5,v11,v12,v13,v14,v15)
{
  ...
  # steps 181-248 for finding coefficients of the polynomial
  m1<-mean(g1);m2<-mean(g2);m3<-mean(g3);m4<-mean(g4);m5<-mean(g5)
  ...
  u7<-d3+d4*h4+h3
  u8<-d4+h4
  # steps 249-258 give coefficients of the polynomial
  z1<-n1*m1*q1+n2*m2*r1+n3*m3*s1+n4*m4*t1+n5*m5*u1
  z2<-n1*(m1*q2-q1)+n2*(m2*r2-r1)+n3*(m3*s2-s1)+n4*(m4*t2-t1)+n5*(m5*u2-u1)
  z3<-n1*(m1*q3-q2)+n2*(m2*r3-r2)+n3*(m3*s3-s2)+n4*(m4*t3-t2)+n5*(m5*u3-u2)
  z4<-n1*(m1*q4-q3)+n2*(m2*r4-r3)+n3*(m3*s4-s3)+n4*(m4*t4-t3)+n5*(m5*u4-u3)
 ...
 # find the roots of the polynomial
  ...
  n=length(Rert)
  # find the n values of sigma_1^2 corresponding to the real roots
  m8=NULL
  for(i in 1:n)
  {
      m8[i]=(sum((g1-Rert[i])^2))/n1
  ...
  m81=min(m8)
  # find the n values of sigma_2^2 corresponding to the real roots
  m9=NULL
  for(i in 1:n)
  {
      m9[i]=(sum((g2-Rert[i])^2))/n2
  ...
  # gives the MLE of sigma_2^2
  m91=min(m9)
  # find the n values of sigma_3^2 corresponding to the real roots
  m10=NULL
  for(i in 1:n)
  {
      m10[i]=(sum((g3-Rert[i])^2))/n3
  ...
  # MLE of sigma_3^2
  m101=min(m10)
  # find the n values of sigma_4^2 corresponding to the real roots
  m11=NULL
  for(i in 1:n)
  {
      m11[i]=(sum((g4-Rert[i])^2))/n4
  ...
  # gives the MLE of sigma_5^2
  m121=min(m12)
  # steps 298-324 are for finding MLEs over the alternative space
  x0<-c(m1, m2, m3, m4, m5)
  s21<-sum((g1-m1)^2)/n1
  s22<-sum((g2-m2)^2)/n2
  s23<-sum((g3-m3)^2)/n3
  ...
  # give the critical value
  out=fun2(n1,n2,n3,n4,n5,v1,v2,v3,v4,v5)
  neu<-(s0[1]^(n1/2))*(s0[2]^(n2/2))*(s0[3]^(n3/2))*(s0[4]^(n4/2))*(s0[5]^(n5/2))
  dno<-(m81^(n1/2))*(m91^(n2/2))*(m101^(n3/2))*(m111^(n4/2))*(m121^(n5/2))
  # give the LR value
  value=neu/dno
  # count if the LR value is less than the critical value
 a=0
 if(value<out)
   a=a+1
 return(a)
